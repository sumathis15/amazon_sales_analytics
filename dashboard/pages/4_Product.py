...
from dashboard.utils import inject_theme, show_chart, apply_filters, inr, load_products, load_transactions
...
show_chart(px.bar(prod.nlargest(15, "revenue"), x="product_name", y="revenue", color="subcategory", title="Top 15 products by revenue"))
...
show_chart(px.area(share, x="order_year", y="share", color="brand", title="Brand share among top 8"))
...
show_chart(px.line(monthly, x="year_month", y="units", color="subcategory", title="Monthly unit demand by subcategory"))
...
show_chart(px.bar(season, x="order_month", y="quantity", title="Seasonal unit demand (month of year)", color_discrete_sequence=[AMAZON_ORANGE]))

st.header("Q19 · Ratings & reviews")
show_chart(px.histogram(tx, x="product_rating", nbins=20, title="Product rating distribution", color_discrete_sequence=[AMAZON_NAVY]))
rated = prod.dropna(subset=["customer_rating"])
show_chart(px.scatter(rated, x="rating", y="revenue", color="subcategory", size="units", title="Catalog rating vs revenue", log_y=True))
...
show_chart(px.bar(launch_perf, x="launch_year", y="first_year_revenue", color="subcategory", title="First-year revenue of newly launched SKUs"))
