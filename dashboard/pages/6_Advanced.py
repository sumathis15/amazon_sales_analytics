...
if cust["churned"].nunique() == 2 and len(cust) > 200:
    ...
    st.plotly_chart(px.histogram(cust, x="churn_prob", color="rfm_segment", title="Predicted churn probability"), use_container_width=True)
else:
...
st.plotly_chart(px.line(brand[brand["brand"].isin(top)], x="order_year", y="final_amount_inr", color="brand", title="Brand competitive set"), use_container_width=True)
price = tx.groupby(["subcategory", "brand"], as_index=False)["discounted_price_inr"].median()
st.plotly_chart(px.box(tx[tx["brand"].isin(top)], x="brand", y="discounted_price_inr", color="subcategory", title="Price architecture by brand"), use_container_width=True)
...
st.plotly_chart(px.density_heatmap(assoc, x="to", y="from", z="confidence", title="P(buy to | bought from)"), use_container_width=True)
st.dataframe(assoc.head(12), use_container_width=True)

st.header("Q29 · Seasonal planning")
month_cat = tx.groupby(["order_month", "subcategory"], as_index=False)["quantity"].sum()
st.plotly_chart(px.density_heatmap(month_cat, x="order_month", y="subcategory", z="quantity", title="Unit demand calendar"), use_container_width=True)
fest = tx.groupby(["order_month", "festival_name"], as_index=False)["final_amount_inr"].sum().dropna()
st.plotly_chart(px.bar(fest, x="order_month", y="final_amount_inr", color="festival_name", title="Promotional calendar · festival GMV by month"), use_container_width=True)
st.markdown(
    """
...
c1, c2 = st.columns(2)
with c1:
    st.plotly_chart(px.line(by_year.reset_index(), x="order_year", y="revenue", markers=True, title="Command center · revenue"), use_container_width=True)
with c2:
    mix = tx.groupby("subcategory", as_index=False)["final_amount_inr"].sum()
    st.plotly_chart(px.pie(mix, names="subcategory", values="final_amount_inr", title="Command center · mix", hole=0.45), use_container_width=True)
