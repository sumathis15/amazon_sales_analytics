...
show_chart(px.histogram(tx, x="delivery_days", color="delivery_type", barmode="overlay", title="Delivery days by type"))
...
show_chart(px.bar(geo, x="customer_state", y="avg_days", color="customer_tier", barmode="group", title="Average delivery days by state and tier"))

st.header("Q22 · Payment analytics")
pay = tx.groupby(["order_year", "payment_method"], as_index=False).size()
show_chart(px.area(pay, x="order_year", y="size", color="payment_method", groupnorm="fraction", title="Payment mix over time"))
...
show_chart(px.bar(by_cat, x="subcategory", y="return_rate", title="Return rate by subcategory", color_discrete_sequence=[AMAZON_ORANGE]))
...
show_chart(px.bar(csat, x="subcategory", y="customer_rating", title="CSAT (mean customer rating)", range_y=[3, 5], color_discrete_sequence=[AMAZON_NAVY]))
...
show_chart(px.scatter(vendor, x="avg_days", y="return_rate", size="revenue", hover_name="brand", title="Vendor proxy: speed vs returns (size = revenue)"))
...
show_chart(px.bar(state_rel.reset_index(), x="customer_state", y="on_time", title="On-time rate by destination state"))
