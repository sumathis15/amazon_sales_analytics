...
st.plotly_chart(px.bar(freq_band, x="band", y="customers", title="Purchase frequency bands", color_discrete_sequence=[AMAZON_NAVY]), width="stretch")
...
st.plotly_chart(px.sunburst(seq.head(40), path=["first_category", "latest_category"], values="customers", title="First → latest subcategory (switchers)"), width="stretch")
...
st.plotly_chart(px.line(yearly, x="order_year", y="aov", color="is_prime_member", markers=True, title="AOV Prime vs non-Prime"), width="stretch")
...
st.plotly_chart(px.imshow(retention, aspect="auto", color_continuous_scale="YlOrBr", title="Annual cohort retention"), width="stretch")
...
st.plotly_chart(px.bar(age, x="customer_age_group", y="final_amount_inr", color="subcategory", title="Spending by age × subcategory"), width="stretch")
geo_age = tx.groupby(["customer_tier", "customer_age_group"], as_index=False)["final_amount_inr"].mean()
st.plotly_chart(px.density_heatmap(geo_age, x="customer_age_group", y="customer_tier", z="final_amount_inr", title="AOV heatmap: tier × age"), width="stretch")
