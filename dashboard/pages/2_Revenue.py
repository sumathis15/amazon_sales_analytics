...
st.plotly_chart(px.bar(brand, x="brand", y="final_amount_inr", title=f"{drill} · brand drill-down", color_discrete_sequence=[AMAZON_ORANGE]), use_container_width=True)
...
st.dataframe(fest.sort_values("revenue", ascending=False), use_container_width=True)
